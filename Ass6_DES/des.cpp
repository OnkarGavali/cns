...
int main()
{
	...
	//my part
	cout<<"Enter plain text (only 8 characters): ";
	cin>>pt;
	cout<<"Enter key(only 8 characters): ";
	cin>>key;
	if(pt.length()<8 || key.length()<8){
	    cout<<"Please enter pt amd key of length 8\n";
	    return 0;
	}else if(pt.length()>8 || key.length()>8 ){
	    pt=pt.substr(0,8);
	    key=key.substr(0,8);
	    cout<<"Plain text: "<<pt<<endl<<"key : "<<key<<endl;
	}
    pt=ASCIItoHEX(pt);
    key=ASCIItoHEX(key);
	...
	cout << "\nPlain Text in hexadecimal: " << text << endl;
	
	cout << "\nPlain Text in ascii: " << hexToASCII(text) << endl;
}
