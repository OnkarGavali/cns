...
string encrypt(string plain_text, vector<string> rkey_bin,
      vector<string> rkey_hex)
{
  ...
  cout << "After initial permutation: " << bin2hex(plain_text)
    << endl;
  ...
  for (int i = 0; i < 16; i++) {
    ...
    string x = xor_(rkey_bin[i], right_expanded);
...
int main()
{
  ...
  vector<string> rkey_bin; // rkey_bin for RoundKeys in binary
  vector<string> rkey_hex; // rkey_hex for RoundKeys in hexadecimal
  for (int i = 0; i < 16; i++) {
    // Shifting
    ...
    rkey_bin.push_back(RoundKey);
    rkey_hex.push_back(bin2hex(RoundKey));
  }

  cout << "\nEncryption:\n\n";
  string cipher = encrypt(plain_text, rkey_bin, rkey_hex);
  cout << "\nCipher Text: " << cipher << endl;

  cout << "\nDecryption\n\n";
  reverse(rkey_bin.begin(), rkey_bin.end());
  reverse(rkey_hex.begin(), rkey_hex.end());
  string text = encrypt(cipher, rkey_bin, rkey_hex);
